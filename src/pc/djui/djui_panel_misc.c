...
void djui_panel_misc_create(struct DjuiBase* caller) {
    ...
    struct DjuiBase* body = djui_three_panel_get_body(panel);
    {
        ...
        // save storage can only be switched when no game is running
        if (gDjuiInMainMenu) {
            djui_button_create(body, DLANG(MISC, SAVE_STORAGE), DJUI_BUTTON_STYLE_NORMAL, djui_panel_save_storage_create);
        }
